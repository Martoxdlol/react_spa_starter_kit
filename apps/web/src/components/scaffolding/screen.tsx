...
export function Screen(props: ComponentProps<'div'>) {
    ...
    return (
        <div {...props} className={cn('fixed bottom-0 left-0 right-0 top-0', props.style)}>
            <div className='relative mt-[var(--scroll-margin-top)] h-[var(--screen-height)] w-full overflow-hidden'>{props.children}</div>
        </div>
    )
}
